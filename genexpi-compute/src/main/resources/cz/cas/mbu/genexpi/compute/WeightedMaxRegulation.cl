...
#define COMPUTATION_NAME WeightedMaxRegulation

#define KERNEL_NAME(X) X ## WeightedMaxRegulation
...
T_Value CalculateRegulatoryInput(BASE_PARAMS_DEF,
    ...
    GET_IDS 
    T_Value regulatorMax = -INFINITY;
    for(int regulator = 0; regulator < NUM_REGULATORS; regulator++)
    {
    	regulatorMax = max(regulatorMax, W_VALUE(regulator) * REGULATOR_VALUE(regulator, time));
    }
	return(regulatorSum);
}
