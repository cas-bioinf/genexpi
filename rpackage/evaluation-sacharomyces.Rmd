...
yeastract_filtered <- yeastract_df%>% filter(regulator %in% regulators) %>%
  ...
  group_by(regulator) %>% sample_n(10)
...
for(regulator in regulators) {
  cat("======", regulator, "=====\n")
  regulon_to_test <- yeastract_filtered %>% filter(regulator == get("regulator")) %>%  # Using get to take refer to the regulator local variabl (not the column)
    get("target", .)
  evaluation_res <- evaluateRandomForRegulon(deviceSpecs = deviceSpecs,rawProfiles = gds38_raw_to_test, rounds = randomRounds, regulatorName = regulator, regulonNames = regulon_to_test, checkConstantSynthesis = FALSE, time = time_smooth, rawTime = time_raw, randomScale = randomScale, randomLength = randomLength, splineDFs = splineDF, splineIntercept = TRUE, errorDef = errorDef)
  cat("True ratio: ", evaluation_res$trueRatio,"\n")
  cat("Mean random ratio: ", mean(evaluation_res$randomRatios), "\n")
  results_genexpi[[regulator]] <- evaluation_res
}
```

```{r}
results_aracne <- list()
for(regulator in regulators[2]) {
  cat("======", regulator, "=====\n")
  regulon_to_test <- yeastract_filtered %>% filter(regulator == get("regulator"))%>%  # Using get to take refer to the regulator local variabl (not the column)
    get("target", .)
  res_aracne <- evaluateTDAracne(randomRounds, gds38_raw, time_raw, c(splineDF,NULL), time_smooth, randomScale, randomLength, regulatorName = regulator, regulonNames = regulon_to_test, errorDef = errorDef, numBins = 10)
  results_aracne[[regulator]] <- res_aracne
}
```
