...
for(regulator_to_test in regulators) {
  ...
  if(nrow(regulon_to_test_df) > 50) {
    regulon_to_test_df <- regulon_to_test_df %>% sample_n(50)
  }
  regulon_to_test <- regulon_to_test_df$target %>% as.character()
...
for(regulator in regulators) {
  ...
  if(!is.null(results_aracne[[regulator]])) {
    ...
    results_formatted[[step]] <- data.frame(regulator = regulator, type = "Downstream", true = res_aracne$trueRatioDownstream, random = res_aracne$overallRandomRatioDownstream)
    step <- step + 1
    results_formatted[[step]] <- data.frame(regulator = regulator, type = "Connected", true = res_aracne$trueRatioConnected, random = res_aracne$overallRandomRatioConnected)
    step <- step + 1
  }
}
